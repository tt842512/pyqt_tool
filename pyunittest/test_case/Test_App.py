...
import warnings
import json
from pyunittest.test_case import GetToken
...
class Servicepatrol_suite(unittest.TestCase):
    '''maiomiaomiaomaioamiaomiaoamnaio'''
    def setUp(self):
    ...
    def paramget(param, toget): #用于提取前一个接口的某一个返回值
        temp = re.findall('(?<="%s": ").*?(?=")' % toget, json.dumps(param))
        returntemp = temp[0]
        return returntemp

    #测试参数和测试验证的字典
    @file_data("./Test_Data/filename.json")
    # @file_data("./Test_Data/Recovery_dict.json")
    @test.method_automation(dict(classname="TEST_APP"))
    def TEST_APP(self,**kwargs):
        get = self.paramget()
        '''maiomiaomiaomiuaomiuaom'''
